"""OSCAR cross-validation for Camden's AirSense 11 AutoSet (72 nights).

Prerequisites:
  1. Import ``~/ZedProjects/sleepData/tmpdata/cam/`` into OSCAR.
  2. Export a Summary CSV via File → Export → CSV Export Wizard.
     Save to ``~/ZedProjects/sleepData/validation/OSCAR_ResMedCam_Summary_<Date>.csv``.
...
def test_resmed_cam_accuracy(
    sample_paths: dict[str, Path],
    oscar_csv: Callable[[str], Path],
    report_dir: Path,
) -> None:
    """Validate AHI, pressure, and usage against OSCAR for Camden's device.
