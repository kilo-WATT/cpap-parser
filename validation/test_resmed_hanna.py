"""OSCAR cross-validation for Hanna's AirSense 11 AutoSet (45 nights).

Prerequisites:
  1. Import ``~/ZedProjects/sleepData/tmpdata/hanna/`` into OSCAR.
  2. Export a Summary CSV via File → Export → CSV Export Wizard.
     Save to ``~/ZedProjects/sleepData/validation/OSCAR_ResMedHanna_Summary_<Date>.csv``.
...
def test_resmed_hanna_accuracy(
    sample_paths: dict[str, Path],
    oscar_csv: Callable[[str], Path],
    report_dir: Path,
) -> None:
    """Validate AHI, pressure, and usage against OSCAR for Hanna's device.
